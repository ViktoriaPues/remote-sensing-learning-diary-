...
```{r, message=FALSE, include_graphics4, out.width='50%', fig.cap='Figure 1: Dharavi in Mumbai', fig.scap='Image Source: A.Savin via Wikimedia Commons }
knitr::include_graphics('figures/w3p1.png')
```
I looked into a few studies that use remote sensing to map informal settlements and turns turns out its "a mammoth task due to the spatial heterogeneity of urban landscape components, requiring complex analytical processes." @matariraGoogleEarthEngine2022 
...
It is great that the data used in this study is free. However, it still does not seem very accessible to a local government in a LMIC because technical capabilities to procure, let alone conduct such a study seem immense. It would be great if there was a spatial application that allowed city governments to map slums in their city in an automated way or at least with less technical requirement. Ollie Ballinger actually managed to built an intresting Google Earth Engine Application to map slums in Dar es Salam in Tansania. It is different to the approach taken in Durban because he remove the formal buidlings and than works applies classification only on the remaining areas.


```{r, message=FALSE, include_graphics5, out.width='50%', fig.cap='Figure 1: Dharavi in Mumbai', fig.scap='Image Source: A.Savin via Wikimedia Commons }
...
I was not aware how difficult it is to map informal settlements with remote sensing data. I actually assumed it would be one of the easier methods as it is so well known. I can't say that I completly understood the method used in Durban. However, it was still a worth wile excersise for me because it gave me a better idea of what the workflow for remote sensing. 
It seems to be roughly: Choosing and downloading the images, making corrections on it, identifying the relevant features (spectral and texture), classification, validating and testing accuracy, spatial analysis (e.g. regression).
