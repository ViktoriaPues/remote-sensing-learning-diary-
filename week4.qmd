...
Several recent (e.g. from 2023) cloud-free images of the city should be obtained with a resolution of 30m. The relevant bands include: band 1-coastal aerosol, band 2-blue, band 3-green, band 4-red, band 5-near infrared (NIR), and bands 6 & 7-shortwave infrared (SWIR). The classification of land cover types could be done using Random Forest (RF) classifier combined available survey data of the city's informal settlements and Open Street Map data. If no survey data exists, the local government could identify known informal areas that can be manually identified on the image and used for training the classifier. The sample should cover approximately 0.25% of the total study area to reach high accuracy. @UsingOpenData
