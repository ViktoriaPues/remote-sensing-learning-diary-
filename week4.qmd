...
I see two ways in which the heat threshold study could be taken forward to help the City Cooperation better mitigate and adapt to extreme heat:

-   Forcast heat stress and accounting for the effects climate change applying different climate change scenarios according to the IPCC.
...
The classification was done using Random Forest (RF) classifier through
two complementary approaches to develop the training set. In the first
approach, available survey data sets (Jakarta\'s informal settlements map
for 2015) and visual interpretation using High-Resolution Google Map
imagery have been used to build the training set. Throughout the second
round of classification, OpenStreetMap (OSM) layers were used as the
complementary approach for training. Results from the validation test
for the second round revealed better accuracy and precision in
classification.
...
For me, it was an interesting exercise to go through these really challenging methodologies. When working with clients on projects we sometimes get asked to do some "little" additional pieces of analysis, such as mapping slum areas. This is a great example of scope creep (adding seemingly small tasks to a project when already underway). If you're not the technical expert, which both client and project manager are usually not, it can happen that substantial additional tasks get agreed.

This exercise showed me how important it is to always understand exactly the method needed before promising and

I think this may also be due to the fact that the topic of "informal settlement mapping through remote sensing"
