...
```{r, echo=FALSE, include_graphics1, out.width='100%', fig.cap='Figure 1: Electric and magnetic spectrum', fig.scap='Remote Sensing for Dummies', cache=FALSE}
...
While still used in this sector, today remote sensing has been adopted in a range of other disciplines including:
...
@mehmoodMappingFloodAreas2021 discuss that the accuracy of the classifier could be improved by training the same model on sentinel data. Sentinel data has a higher spatial resolution than landsat (10m vs. 30m). Sentinel and Landset have different revisit periods. Using both data sources can increase the frequency of images of the same point over time. A weakness of the study is that it does not consider socio-demographic data to understand flood vulnerability.
